...
(function () {
  ...
  function tryMake(mode, band, pool, used, opening) {
    var f;
    /* Tone rule: a body count is never the dial on a slider, never the wager,
       and never one of the three questions that open a round. */
    ...
    if (mode === 'wager') {
      f = firstFree(pool, used, okNum);
      return f ? makeOverUnder(f, band, 'wager') : null;
    }
  ...
  function buildRound(catId, gentle) {
    ...
    return { catId: catId, questions: qs, i: 0, score: 0, streak: 0, results: [], stake: 0 };
  }
  ...
  function renderQuestion() {
    ...
    $('band').textContent = q.mode === 'wager' ? 'Final question' : BAND_LABEL[q.band];
  ...
  function renderActions(q) {
    ...
    if (q.mode === 'wager' && R.stake === 0 && !q.staked) {
      var label = document.createElement('p');
      label.className = 'stake-label';
      label.textContent = 'Stake up to half your ' + R.score.toLocaleString('en-US') + ' points';
      a.appendChild(label);

      var row = document.createElement('div');
      row.className = 'row';
      [0, 0.25, 0.5].forEach(function (pct) {
        var amount = Math.round(R.score * pct);
        var b = document.createElement('button');
        b.className = 'ans brass';
        b.type = 'button';
        b.innerHTML = pct === 0 ? 'No bet' : (pct * 100) + '%<br>' + amount.toLocaleString('en-US');
  ...
  function answer(given) {
    ...
    var swing = 0;
    if (q.mode === 'wager' && R.stake > 0) {
      swing = correct ? R.stake : -R.stake;
    }

    R.score = Math.max(0, R.score + earned + swing);
    R.results[R.i] = {
      grade: grade, earned: earned, swing: swing, given: given,
      offBy: offBy, q: q
    };
  ...
  function openSheet(q, res, bonus) {
    ...
    var total = res.earned + res.swing;
    var ptsText = (total >= 0 ? '+' : '') + total.toLocaleString('en-US');
